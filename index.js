...
function buildSystemMessage(metadata = null, language = 'en-US') {
  ...
  if (metadata) {
    if (metadata.firstName) {
      prospectName = metadata.firstName;
      if (metadata.lastName) {
        prospectName += ` ${metadata.lastName}`;
      }
      contextInfo.push(`You're calling ${prospectName}`);
    }
    
    if (metadata.company) {
      companyName = metadata.company;
      if (contextInfo.length > 0) {
        contextInfo[contextInfo.length - 1] += ` from ${metadata.company}`;
      } else {
        contextInfo.push(`You're calling someone from ${metadata.company}`);
      }
    }
    
    if (metadata.jobTitle) {
      roleInfo = ` who is the ${metadata.jobTitle}`;
      if (contextInfo.length > 0) {
        contextInfo[contextInfo.length - 1] += roleInfo;
      }
    }

    if (metadata.industry) {
      industryInfo = ` in the ${metadata.industry} industry`;
      if (contextInfo.length > 0) {
        contextInfo[contextInfo.length - 1] += industryInfo;
      }
    }

    if (metadata.painPoints) {
      contextInfo.push(`Known pain points: ${metadata.painPoints}`);
    }
    
    if (metadata.budget && metadata.budget !== 'unknown') {
      contextInfo.push(`Estimated budget range: ${metadata.budget}`);
    }
    
    if (metadata.timeline) {
      contextInfo.push(`Decision timeline: ${metadata.timeline}`);
    }
    
    if (metadata.leadSource) {
      contextInfo.push(`Lead source: ${metadata.leadSource}`);
    }
    
    if (metadata.previousContact) {
      contextInfo.push(`Previous contact history: ${metadata.previousContact}`);
    }
    
    if (metadata.callObjective) {
      contextInfo.push(`Call objective: ${metadata.callObjective}`);
    }
    
    if (metadata.additionalNotes) {
      contextInfo.push(`Additional notes: ${metadata.additionalNotes}`);
    }
    
    if (metadata.priority) {
      contextInfo.push(`Lead priority: ${metadata.priority}`);
    }
  }

  /* ---------- SALES AGENT personality & flow ---------- */
  let baseMessage = [
    "You are Alex, a professional and enthusiastic sales agent who specializes in qualifying leads and scheduling product demos.",
    "You're personable, confident, and genuinely curious about helping prospects solve their business challenges.",
    "You listen carefully, ask thoughtful questions, and build rapport naturally without being pushy or aggressive.",
    "You're calling on behalf of your company to follow up on a lead and potentially schedule a product demonstration.",
    "Keep your language professional but conversational—no jargon, abbreviations, or overly sales-y phrases.",
    "Do not speak until you hear the prospect greet you first. When you reply, follow this structured approach:",
    `1. GREETING & INTRODUCTION • Say: Hi ${prospectName}! This is Alex calling from [your company name]. I hope I'm catching you at a good time. I'm reaching out because you showed interest in our [product/service]. Is this still a good number to reach you on? • **Wait for response.**`,
    "2. PERMISSION & RAPPORT • Say: Great! I know your time is valuable, so I'll be brief. I was hoping to learn a bit more about [company name] and see if there might be a good fit for what we do. Would you have just a couple of minutes to chat? • **Wait for response.**",
    "3. DISCOVERY & QUALIFICATION • Ask about their current challenges, what solutions they're currently using, and what their ideal outcome would look like. Listen actively and ask follow-up questions. Be genuinely curious about their business. • **Stop and wait for responses.**",
    "4. VALUE PROPOSITION • Based on what they share, briefly explain how your solution could specifically help address their challenges. Keep it relevant to what they just told you. • **Wait for response.**",
    "5. DEMO SCHEDULING OR NEXT STEPS • Say: Based on what you've shared, I think a quick demo would be really valuable to show you exactly how this could work for [company name]. I have some time [suggest specific times]. Would either of those work for you? • **Wait for response.**",
    "6. CLOSE • Confirm next steps, get their email if needed, and thank them for their time. Let them know you'll send a calendar invite with all the details."
  ];  

  if (contextInfo.length) {
    baseMessage.splice(4, 0, contextInfo.join('. ') + '.');
  }
  
  baseMessage.push(`Always reply in ${language}.`);
  baseMessage.push("Remember: Your goal is to qualify the lead and schedule a demo, not to close a sale on this call.");
  baseMessage.push("Be helpful, professional, and respectful. If they're not interested, thank them and end the call politely.");
...
fastify.post('/api/call', async (request, reply) => {
  console.log('>>> /api/call endpoint hit <<<');
  try {
    // Set CORS headers
    ...
    // Parse request body - sales agent data
    console.log('Raw request body:', request.body);
    const salesData = request.body;

    if (!salesData.phoneNumber) {
    ...
    if (!salesData.firstName || !salesData.company || !salesData.industry) {
      console.log('Error: Required fields missing');
      return reply.code(400).send({
        error: 'First name, company, and industry are required'
      });
    }

    const language = 'en-US'; // Default language for sales calls
    console.log(`Received sales call request for: ${salesData.firstName} ${salesData.lastName} at ${salesData.company}`);
    console.log('Sales data received:', JSON.stringify(salesData, null, 2));

    // Transform sales data into metadata format
    const metadata = {
      firstName: salesData.firstName,
      lastName: salesData.lastName,
      company: salesData.company,
      jobTitle: salesData.jobTitle,
      industry: salesData.industry,
      companySize: salesData.companySize,
      leadSource: salesData.leadSource,
      priority: salesData.priority,
      painPoints: salesData.painPoints,
      budget: salesData.budget,
      timeline: salesData.timeline,
      previousContact: salesData.previousContact,
      callObjective: salesData.callObjective || 'qualification',
      additionalNotes: salesData.additionalNotes,
      urgentCall: salesData.urgentCall === 'on',
      sendSummary: salesData.sendSummary === 'on',
      email: salesData.email,
      prospectName: `${salesData.firstName} ${salesData.lastName}`.trim()
    };

    // Send email data to N8N webhook if email is provided and webhook URL is configured
    if (metadata.email && N8N_EMAIL_WEBHOOK_URL) {
      try {
        console.log(`Sending sales lead data to N8N webhook for: ${metadata.email}`);
        const emailData = {
          email: metadata.email,
          phoneNumber: salesData.phoneNumber,
        ...
        if (emailResponse.ok) {
          console.log('Sales lead data sent to N8N successfully');
        } else {
          console.error('Failed to send sales lead data to N8N:', await emailResponse.text());
        }
      } catch (emailError) {
        console.error('Error sending sales lead data to N8N:', emailError.message);
    ...
    // Initiate the call
    try {
      const call = await makeCall(salesData.phoneNumber, language);

      if (!call) {
        console.log(`Failed to initiate call to ${salesData.phoneNumber} - not allowed or other error`);
        return reply.code(400).send({
          error: 'Could not initiate call, check if the number is allowed'
        });
      }

      console.log(`Successfully initiated sales call to ${salesData.phoneNumber}, SID: ${call.sid}`);

      // Store metadata for this call
      callMetadata.set(call.sid, metadata);
      console.log(`Stored sales metadata for call ${call.sid}:`, JSON.stringify(metadata, null, 2));
      ...
      console.error(`Error making sales call to ${salesData.phoneNumber}:`, callError);
      return reply.code(500).send({
        error: `Error making call: ${callError.message}`
      });
    }
  } catch (error) {
    console.error('!!! Error in /api/call endpoint:', error.message);
    return reply.code(500).send({
      error: 'Error initiating call: ' + error.message
    });
  }
});

// Get call status endpoint for sales agent
fastify.get('/api/call/status/:callSid', async (request, reply) => {
  ...
  try {
    ...
    return {
      sid: call.sid,
      status: call.status,
      prospectName: metadata.prospectName || 'Unknown',
      to: call.to,
      duration: call.duration,
      startTime: call.startTime,
      endTime: call.endTime,
      outcome: call.status === 'completed' ? 'Call completed' : 'Pending'
    };
  } catch (error) {
    fastify.log.error(error);
